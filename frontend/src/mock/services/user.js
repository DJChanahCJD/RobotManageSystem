...
const userList = (options) => {
  ...
  const generateData = (count) => {
    ...
    for (let i = 0; i < count; i++) {
      result.push({
        ...
        authority: Mock.mock('@pick(["admin", "user"])'),
  ...
  const filteredData = totalData.filter(item => {
    ...
    if (parameters.phone && !item.phone.includes(parameters.phone)) return false
    if (parameters.authority && item.authority !== parameters.authority) return false
    return true
  })
...
Mock.mock(/\/user\/delete\/\d+/, 'delete', () => {
  return builder({}, '删除成功')
})
